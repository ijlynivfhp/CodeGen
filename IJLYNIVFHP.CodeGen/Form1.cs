    ...
    public partial class Form1 : Form
    {
        ...
        private void btndalyl_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (tmp == "Sqlite")
                {
                    txtyl.Text = GenDAL_SQLite_IJLYNIVFHP.GenAllCode(ns, tabname, classname, connstr);
                }
                if (tmp == "MySQL")
                {
                    txtyl.Text = GenDAL_MySQL_IJLYNIVFHP.GenAllCode(ns, tabname, classname, connstr);
                }
                if (tmp == "Access")
                {
                    txtyl.Text = GenDAL_Access_IJLYNIVFHP.GenAllCode(ns, tabname, classname, connstr);
                }
                if (tmp == "SQL Server")
                {
                    txtyl.Text = GenDAL_MSSQL_IJLYNIVFHP.GenAllCode(ns, tabname, classname, connstr);
                }
            }
        ...
        private void cobdbtype_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (cobdbtype.SelectedItem.ToString())
            {
                case "SQL Server":
                    rad0.Enabled = true;
                    txtyl.Text = @"示例连接字符串: server=192.168.0.135;uid=sa;pwd=Famous901;database=LaborSys;pooling=true;min pool size=5;max pool size=100;";
                    break;
                case "MySQL":
                    rad0.Enabled = false;
        ...
        /// <param name="e"></param>
        private void btnGenList1_Click(object sender, EventArgs e)
        {
            if (lsbright.Items.Count ==0)
            {
            ...
            string front = txtfront.Text.Trim();
            string connstr = txtconnstr.Text.Trim(); // 数据库连接字符串
            txtyl.Text = new GenList_MSSQL().List1(tabname,front, connstr);
        }

        private void btnGenList2_Click(object sender, EventArgs e)
        {
            if (lsbright.Items.Count == 0)
            {
                MessageBox.Show("请选择需要操作的表！");
           ...
           string  classname = tabname;  //类名
            ...
            classname = classname.Substring(0, 1).ToUpper() + classname.Substring(1);
            txtyl.Text = new GenList_MSSQL().List2(classname);
        }

        private void btnGenDel_Click(object sender, EventArgs e)
        {
            if (lsbright.Items.Count == 0)
            {
                MessageBox.Show("请选择需要操作的表！");
            ...
            classname = classname.Substring(0, 1).ToUpper() + classname.Substring(1);
            txtyl.Text = new GenList_MSSQL().Del(classname);
        }

        private void btnGenAdd1_Click(object sender, EventArgs e)
        {
            if (lsbright.Items.Count == 0)
            {
                MessageBox.Show("请选择需要操作的表！");
            ...
            string front = txtfront.Text.Trim();
            string connstr = txtconnstr.Text.Trim(); // 数据库连接字符串
            txtyl.Text = new GenList_MSSQL().Add1(tabname, front, connstr);
        }

        private void btnGenAdd2_Click(object sender, EventArgs e)
        {
            if (lsbright.Items.Count == 0)
            {
                MessageBox.Show("请选择需要操作的表！");
            ...
            string front = txtfront.Text.Trim();
            string connstr = txtconnstr.Text.Trim(); // 数据库连接字符串
            txtyl.Text = new GenList_MSSQL().Add2(tabname, front, connstr);
        }

        private void txtyl_KeyPress(object sender, KeyPressEventArgs e)
        {
